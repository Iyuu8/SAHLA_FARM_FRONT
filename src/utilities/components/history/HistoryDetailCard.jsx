...
import { SunnyIcon, ClearNightIcon, CloudyIcon, HeavyRainIcon, HeavyRainAltIcon, WindyIcon } from '../../data/Icons';
import {
  convertHumidity,
...
const ActuatorCard = ({ actuator }) => {
    ...
    if (control_mode === "auto") {
      if (startTime && endTime) return (
        <div className="flex flex-col md:gap-1 flex-wrap">
          ...
          <p><span className="text-white font-bold text-[11px] md:text-[16px]">{startTime}</span><span className='text-white font-extrabold text-[13px] md:text-[20px]'> → </span><span className="text-white font-bold text-[11px] md:text-[16px]">{endTime}</span></p>
        </div>
      );
      if (startTime) return (
      ...
      return (
        <div className="flex flex-col md:gap-1">
          <span className={`text-[10px] md:text-[14px] opacity-80 ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>{t('history.actuatorCard.noPlannedTime')}</span>
        </div>
      );
    }

    if (control_mode === "semi_auto") {
      if (startTime && endTime) return (
        <div className="flex flex-col md:gap-1 flex-wrap">
          <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>{t('history.actuatorCard.suggestedFrom')}</span>
          <p><span className="font-bold text-white text-[11px] md:text-[16px]">{startTime}</span><span className='text-white font-extrabold text-[13px] md:text-[20px]'> → </span><span className="font-bold text-white text-[11px] md:text-[16px]">{endTime}</span></p>
        </div>
      );
      if (startTime) return (
        <div className="flex flex-col md:gap-1">
      ...
      return (
        <div className="flex flex-col md:gap-1">
          <span className={`text-[10px] md:text-[14px] opacity-80 ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>{t('history.actuatorCard.noSuggestedTime')}</span>
        </div>
      );
    }
  ...
  return (
    <div className={`flex-1 rounded-2xl mb-2 md:py-3 md:px-4 p-2 ${status === "on" ? "bg-[#3F8806]" : "bg-[#192514]"}`}>
      ...
      {timingDisplay && <div>{timingDisplay}</div>}
    </div>
  );
};
...
export default function HistoryDetailCard({
  ...
  const crop = data.crop?.type || ''
  const growthStage = data.crop?.growth_stage || ''
  const actuators = data.actuators || []
  const weatherSummary = data.weather?.summary || ''
  const aiRecommendation = data.recommendation || ''
  const weatherState = data.weather?.state?.toLowerCase() || 'sunny'
  const WeatherIcon = weatherIconMap[weatherState] || SunnyIcon

  // Map sensors array to values
  const sensorsObj = {}
  for (const s of data.sensors || []) {
    sensorsObj[s.type] = s.value
  }
  ...
  const convertedSensors = {
    ...
    soilMoisture: formatConvertedValue(convertSoilMoisture(soilValue, soilMoistureUnit), soilMoistureUnit, 1),
    lightIntensity: formatConvertedValue(convertLightIntensity(lightValue, lightIntensityUnit), lightIntensityUnit, 1),
  }
  ...
  return (
    <div className="bg-[#F5F7F4] rounded-3xl px-4 pb-5 md:px-7 max-w-2xl w-full shadow-md font-sans mx-auto relative overflow-y-auto max-h-[90vh] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <button onClick={onClose} className="bg-white rounded-full p-2 shadow-md hover:bg-gray-100 transition-colors absolute md:right-4 md:top-4 right-2 top-2">
      ...
      <div className="py-3 md:py-2 flex md:gap-5 items-center md:flex-row">
        {!isMobile && <WeatherIcon color={"#192514"} size={200} />}
        <div className='w-full'>
          <div className='flex gap-4 md:gap-0'>
            {isMobile && <WeatherIcon color={"#192514"} size={40} />}
            <div>
          ...
          <div className='text-[#333333] flex flex-col md:gap-3 gap-2'>
            <div className='grid grid-cols-2 md:gap-6 gap-5'>
              <div className='flex w-full items-center gap-2 text-[#333333] font-newblack bg-[#E0E0E0] border border-[#B4B4B4] md:px-3 px-2 py-2 md:py-3 rounded-lg text-center md:text-[16px] text-[14px] whitespace-nowrap'>
                <TemperatureIcon size={isMobile ? 30 : 24} />
                <div className='flex flex-col-reverse items-start md:flex-row'>
                  <span>{t('history.historyCard.sensors.temperature')}</span>
                  <span className='font-bold text-[16px] md:font-normal'>{!isMobile && <span>:</span>} {convertedSensors.temperature}</span>
                </div>
              </div>
              <div className='flex w-full items-center gap-2 text-[#333333] font-newblack bg-[#E0E0E0] border border-[#B4B4B4] md:px-3 px-2 py-2 md:py-3 rounded-lg text-center md:text-[16px] text-[14px] whitespace-nowrap'>
                <HumidityIcon size={isMobile ? 30 : 24} />
                <div className='flex flex-col-reverse items-start md:flex-row'>
                  <span>{t('history.historyCard.sensors.humidity')}</span>
                  <span className='font-bold text-[16px] md:font-normal'>{!isMobile && <span>:</span>} {convertedSensors.humidity}</span>
                </div>
              </div>
            </div>
            <div className='grid grid-cols-2 md:gap-6 gap-4'>
              <div className='flex w-full items-center gap-2 text-[#333333] font-newblack bg-[#E0E0E0] border border-[#B4B4B4] md:px-3 px-2 py-2 md:py-3 rounded-lg text-center md:text-[16px] text-[14px] whitespace-nowrap'>
                <SoilMoistureIcon size={isMobile ? 30 : 24} />
                <div className='flex flex-col-reverse items-start md:flex-row'>
                  <span>{t('history.historyCard.sensors.soilMoisture')}</span>
                  <span className='font-bold text-[16px] md:font-normal'>{!isMobile && <span>:</span>} {convertedSensors.soilMoisture}</span>
                </div>
              </div>
              <div className='flex w-full items-center gap-2 text-[#333333] font-newblack bg-[#E0E0E0] border border-[#B4B4B4] md:px-3 px-2 py-2 md:py-3 rounded-lg text-center md:text-[16px] text-[14px] whitespace-nowrap'>
                <LightIcon size={isMobile ? 30 : 24} />
                <div className='flex flex-col-reverse items-start md:flex-row'>
                  <span>{t('history.historyCard.sensors.lightIntensity')}</span>
                  <span className='font-bold text-[16px] md:font-normal'>{!isMobile && <span>:</span>} {convertedSensors.lightIntensity}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div>
        ...
        <div className='flex flex-col md:flex-row gap-3'>
          {actuators.map((act, i) => <ActuatorCard key={i} actuator={act} />)}
        </div>
      </div>
