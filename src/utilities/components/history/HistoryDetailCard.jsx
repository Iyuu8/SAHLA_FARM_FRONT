  ...
  const getTimingDisplay = () => {
    ...
    if (control_mode === "auto") {
      ...
      if (startTime && endTime) {
        return (
          <div className="flex flex-col md:gap-1  flex-wrap">
            <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>planned to execute from</span>
            <p>
        ...
        return (
          <div className="flex flex-col md:gap-1 ">
            <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>planned to execute at</span>
        ...
        return (
          <div className="flex flex-col md:gap-1 ">
            <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>planned to execute until</span>
      ...
      if (startTime && endTime) {
        return (
          <div className="flex flex-col md:gap-1 flex-wrap">
            <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>suggested to execute from</span>
            <p>
              <span className="font-bold text-white  text-[11px] md:text-[16px]">{startTime}</span>
              <span className='text-white  font-extrabold text-[13px] md:text-[20px]'> → </span>
              <span className="font-bold text-white  text-[11px] md:text-[16px]"> {endTime}</span>
        ...
        return (
          <div className="flex flex-col md:gap-1">
            <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>suggested to execute at</span>
            <span className="font-bold text-white text-[11px] md:text-[16px]">{startTime}</span>
          </div>
        );
      } else if (endTime) {
        return (
          <div className="flex flex-col md:gap-1">
            <span className={`text-[10px] md:text-[14px] ${status === "on" ? "text-[#E1F9CF]" : "text-white"}`}>suggested to execute until</span>
            <span className="font-bold text-white text-[11px] md:text-[16px]">{endTime}</span>
          </div>
        );
      }
  ...
  return (
    <div className={`flex-1 rounded-2xl mb-2 md:py-3 md:px-4 p-2 ${ status ==="on" ? "bg-[#3F8806]" : "bg-[#192514]"}`}>
      ...
      <div className="flex items-center justify-between md:mb-1">
        <span className="font-normal font-newblack md:text-[24px] text-[15px] capitalize text-[#F8FFF6]">{type}:</span>
  ...
  return (
    <div className="bg-[#F5F7F4] rounded-3xl px-4 pb-2 md:px-7 max-w-2xl w-full shadow-md font-sans mx-auto relative overflow-y-auto max-h-[90vh] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <div className='w-full'>
            ...
            <div>
              ...
              <p className='text-[#295D00] font-newblack flex md:gap-3 gap-3 md:text-[22px] text-[15px] '>
                  <span>{crop} in</span>
                  <span>stage:</span>
                  <span className=''>{growthStage}</span>
              </p>
            </div>
            </div>
      ...
      <div>
        ...
        <div className='flex md:gap-2 gap-1 items-center'>
           <div className='w-[8px] h-[8px] md:w-[12px] md:h-[12px] bg-[#1A3D00] rounded-full md:mb-2 mb-1'></div>
            <div className='font-newblack text-[#1A3D00] font-semibold md:text-[30px] text-[18px]'>Actuators</div>
        </div>
        {/* the pump and fan  */}
      ...
      <div>
        ...
        <div className='flex md:gap-2 gap-1 items-center'>
           <div className='w-[8px] h-[8px] md:w-[12px] md:h-[12px] bg-[#1A3D00] rounded-full md:mb-2 mb-1'></div>
            <p className='font-newblack text-[#1A3D00] font-semibold md:text-[30px] text-[18px]'>Weather Summary</p>
        </div>
        {/* Body */}
        <p
      ...
      <div>
        ...
        <div className='flex md:gap-2 gap-1 items-center'>
           <div className='w-[8px] h-[8px] md:w-[12px] md:h-[12px] bg-[#1A3D00] rounded-full md:mb-2 mb-1'></div>
            <p className='font-newblack text-[#1A3D00] font-semibold md:text-[30px] text-[18px]'>SAHLA AI . Recommendations</p>
        </div>
        {/* Body */}
