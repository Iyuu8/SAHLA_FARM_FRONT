    ...
    return new Promise((resolve) => {
      ...
      img.onload = async () => {
        try {
          ...
          console.error('[ProfilePictureEditorModal] Upload failed:', err);
      ...
      <div className='w-full max-w-[540px] rounded-2xl bg-white p-5 sm:p-6 shadow-[0_10px_34px_rgba(0,0,0,0.28)]'>
        ...
        {error && (
          <div className='mt-4 p-3 bg-[#FEE2E2] border border-[#FECACA] rounded-lg'>
            <p className='text-sm text-[#C73030] font-semibold'>{error}</p>
            <button
              type='button'
              onClick={handleRetry}
              disabled={loading}
              className='mt-2 px-3 py-1 bg-[#C73030] text-white text-sm rounded font-semibold hover:bg-[#B91C1C] disabled:opacity-50 transition-colors'
            >
